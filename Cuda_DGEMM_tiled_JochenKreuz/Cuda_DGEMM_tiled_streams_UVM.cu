...
int main () {
  ...
  double *pb;
  double *pc;
  cudaMallocManaged ( &pa, m*m*sizeof(double)*nstreams );
  cudaMallocManaged ( &pb, m*m*sizeof(double)*nstreams );
  cudaMallocManaged ( &pc, m*m*sizeof(double)*nstreams );
  ...
  // PERFORM MULTIPLICATION
  {
    ...
    for ( int iktile = 0; iktile < ntiles; iktile++ ) {
	  ...
	  // perform dgemm
	  cublasDgemm ( cublasHandle, CUBLAS_OP_T, CUBLAS_OP_T, m, m, m, &alpha, &pa[ibuff*offset], m, &pb[ibuff*offset], m, &beta, &pc[ibuff*offset], m ); 
